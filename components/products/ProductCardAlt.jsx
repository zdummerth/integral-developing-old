...
const Product = ({ product, recommendedProducts }) => {
  ...
  const [currentImage, setCurrentImage] = useState(currentVariant.image);

  const nonVariantImages = images.filter((image) => {
    const variantImageIds = variants.map((v) => v.image.id);
    // console.log(variantImageIds);
    return !variantImageIds.some((id) => id === image.id);
  });

  const viewableImages = [currentVariant.image, ...nonVariantImages];
  const nextImage = () => {
    const currentImageIndex = viewableImages.findIndex(
      (el) => el.id === currentImage.id
    );
    currentImageIndex === viewableImages.length - 1
      ? setCurrentImage(viewableImages[0])
      : setCurrentImage(viewableImages[currentImageIndex + 1]);
  };
  ...
  const prevImage = () => {
    ...
    currentImageIndex === 0
      ? setCurrentImage(viewableImages[viewableImages.length - 1])
      : setCurrentImage(viewableImages[currentImageIndex - 1]);
  };
    ...
    viewableImages.map((image) => (
      <div
        ...
        className={`w-4 h-4 rounded-full ${
          currentImage.id === image.id ? "bg-slate-600" : "bg-slate-200"
        }`}
      ></div>
    ));

  // console.log([currentVariant.image, ...nonVariantImages]);
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 w-full md:p-4">
      <div>
        <div className="relative w-full aspect-[1/1]">
          {viewableImages.map((img) => (
            <div
              key={img.id}
              className={`absolute w-full h-full transition-all duration-500 ease-out ${
                img.id === currentImage.id ? "opacity-100" : "opacity-0"
              }`}
            >
              <Image
                src={img.src}
