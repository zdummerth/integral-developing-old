...
const CartContextProvider = ({ children }) => {
  ...
  const initializeCart = async (reset) => {
    ...
    const existingCartID = isBrowser
      ? localStorage.getItem("stewards_of_christ_cart_id")
      : null;

    const setCartInState = (newCart) => {
      if (isBrowser) {
        localStorage.setItem("stewards_of_christ_cart_id", newCart.id);
      }
      updateCart(newCart);
    };
    ...
    if (existingCartID && existingCartID !== "undefined") {
      try {
        ...
        localStorage.removeItem("stewards_of_christ_cart_id");
        setError((prev) => ({
          ...prev,
      ...
      try {
        ...
        setLoading(true);
        const res = await callApi("/api/shopify-graphql", "POST", {
